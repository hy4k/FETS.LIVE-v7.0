...
const sid = () => F()._meId || F()._meUserId || null;        // staff_attendance.staff_id
...
export async function attToday() {
  const id = sid(); if (!id) return null;
...
export async function attCheckIn(branch) {
  const id = sid(); if (!id) return { error: "We couldn't match your staff profile — ask an admin to link your account." };
...
export async function attStepOut() {
  ...
  try { await supabase.from("staff_attendance").update({ notes: JSON.stringify(meta), updated_at: new Date().toISOString() }).eq("staff_id", sid()).eq("date", attDateStr()); return { ok: true }; }
...
export async function attCheckOut() {
  try { await supabase.from("staff_attendance").update({ check_out: hhmm(), updated_at: new Date().toISOString() }).eq("staff_id", sid()).eq("date", attDateStr()); return { ok: true }; }
  catch (e) { return { error: e.message }; }
}
