...
async function run() {
  console.log("Calling exec_sql RPC via Supabase service role...");
  const { data, error } = await supabase.rpc('exec_sql', { sql: ddl });
  if (error) {
    console.log("exec_sql(sql) failed, trying exec_sql(query)...");
    const { data: data2, error: error2 } = await supabase.rpc('exec_sql', { query: ddl });
    if (error2) {
      console.error("❌ RPC execution failed:", error2);
    } else {
      console.log("✅ Leave/Delete RPC migrations applied successfully via exec_sql(query)!");
    }
  } else {
    console.log("✅ Leave/Delete RPC migrations applied successfully via exec_sql(sql)!");
  }
}
run();
